...
const envSchema = z.object({
  ...
  ZAI_RETRY_ATTEMPTS: z.coerce.number().int().min(1).max(8).default(2),
  ZAI_RETRY_BASE_MS: z.coerce.number().int().min(100).max(15000).default(2000),
  ZAI_RETRY_MAX_MS: z.coerce.number().int().min(500).max(30000).default(10000),
...
export function validateEnv(): Env {
  ...
  try {
    validatedEnv = envSchema.parse({
      ...
      GROQ_MODEL_NR6: process.env.GROQ_MODEL_NR6,
      AI_PROVIDER: process.env.AI_PROVIDER,
      ZAI_API_KEY: process.env.ZAI_API_KEY,
      ZAI_BASE_URL: process.env.ZAI_BASE_URL,
      ZAI_MODEL: process.env.ZAI_MODEL,
      ZAI_DISABLE_THINKING: process.env.ZAI_DISABLE_THINKING,
      ZAI_TIMEOUT_MS: process.env.ZAI_TIMEOUT_MS,
      ZAI_RETRY_ATTEMPTS: process.env.ZAI_RETRY_ATTEMPTS,
      ZAI_RETRY_BASE_MS: process.env.ZAI_RETRY_BASE_MS,
      ZAI_RETRY_MAX_MS: process.env.ZAI_RETRY_MAX_MS,
