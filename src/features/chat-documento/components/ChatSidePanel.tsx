...
export function ChatSidePanel({ isOpen, onClose, documentContext, documentName }: ChatSidePanelProps) {
            ...
            {isOpen && (
                <>
                    <motion.div
                        {...fadeProps}
                        onClick={onClose}
                        className="fixed inset-0 bg-black/30 dark:bg-black/60 backdrop-blur-[2px] z-sgn-overlay hidden sm:block"
                        aria-hidden="true"
                    />
                    ...
                    <motion.div
                        ...
                        className="fixed top-0 right-0 h-full w-full sm:w-[460px] bg-gray-50 dark:bg-[#0a0a0f] z-sgn-tooltip flex flex-col no-print shadow-2xl"
                    >
                        {/* Header */}
