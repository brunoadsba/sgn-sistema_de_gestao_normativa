...
- **Workspace NotebookLM (Sessão 39)**: Migração para layout de 3 colunas paralelas. A largura do container é dinâmica (`1700px` em 3-col vs `max-w-7xl` em 2-col). Chat NEX acessível via popup (`/chat`) a partir do `GlobalNav` ou Cmd/Ctrl+K.
...
## Evolucoes Recentes (2026-03-03)

- **Tela inicial mobile**: AppShell com `py-4` no main; AnaliseCliente com header compacto, espaçamentos reduzidos, stepper e botão responsivos; cards lado a lado apenas em `md:` (768px+). Placeholders de busca abreviados (SeletorNormas "Buscar") para evitar truncamento.

## Evolucoes Anteriores (2026-02-28)

- **Chat sempre acessível**: NEX aberto em popup (`/chat`) via botão no `GlobalNav` ou atalho Cmd/Ctrl+K. Sem painel lateral ou FAB.
- **Backend hardening**: todas as respostas de API padronizadas com `createSuccessResponse`/`createErrorResponse`. Frontend ajustado para ler `data.data.*`.
- **Rate limiting**: todas as rotas de API protegidas com rate limiting in-memory.
- **Testes E2E**: cobertura com Playwright para upload, seletor de normas e APIs.
